...
class RandomRotate(object):
    ...
    def filter_border(self, bboxes, h, w):
        x_all = np.sort(bboxes[:, 0:8:2])
        y_all = np.sort(bboxes[:, 1:8:2])
        keep_inds = (x_all[:,0] > 0) & (x_all[:,3] < w) & (y_all[:,0] > 0) & (y_all[:,3] < h)
        return keep_inds
    ...
    def __call__(self, results):
        ...
        if results['rotate'] == False:
            results['rotate_angle'] = 0
            return results
        h, w, c = results['img_shape']
        img = results['img']
        angle = results['rotate_angle']
