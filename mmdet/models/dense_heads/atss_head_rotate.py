...
from mmdet.core import (cornerbias2rbbox, force_fp32, multi_apply, multiclass_rotate_eight_nms,
                        anchor_target_rotated, anchor_inside_flags, build_assigner, build_sampler,
...
class ATSSHeadRotate(AnchorHeadRotate):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.loss_cls_init = build_loss(loss_cls_init)
    ...
    def _init_layers(self):
        ...
        self.reg_convs_init = nn.ModuleList()
        self.cls_convs_init = nn.ModuleList()
        for i in range(self.stacked_convs):
            ...
            self.cls_convs_init.append(
                ConvModule(
                    ...
                    padding=1,
                    conv_cfg=conv_cfg))
        # self.reg_init = nn.Conv2d(self.feat_channels, 8, 3, padding=1)
        self.reg_init = nn.Conv2d(self.feat_channels, 5, 3, padding=1)
        self.cls_init = nn.Conv2d(self.feat_channels, self.cls_out_channels, 3, padding=1)
    ...
    def init_weights(self):
        ...
        for m in self.reg_convs_init:
            normal_init(m.conv, std=0.01)
        for m in self.cls_convs_init:
            normal_init(m.conv, std=0.01)
        bias_cls = bias_init_with_prob(0.01)
        normal_init(self.reg_init, std=0.01)
        normal_init(self.cls_init, std=0.01, bias=bias_cls)
    ...
    def forward_single(self, x, scale):
        ...
        for cls_layer in self.cls_convs_init:
            cls_feat = cls_layer(cls_feat)

        cls_score = self.cls_init(cls_feat)

        for reg_layer in self.reg_convs_init:
            reg_feat = reg_layer(reg_feat)
        bbox_pred = self.reg_init(reg_feat)
    ...
    def loss(self,
        ...
        labels_init, bbox_targets, _, labels_score, _ = self.get_targets(anchor_list, valid_flag_list,
                                                bbox_preds, r_gt_bboxes_five, r_gt_bboxes_eight,
        ...
        loss_cls_init = self.loss_cls_init(
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, reg_confidence, anchors in zip(
            ...
            # scores = F.softmax(cls_score.permute(1, 2, 0).reshape(-1, self.cls_out_channels))[...,:-1]
            reg_confidence = reg_confidence.permute(1, 2, 0).reshape(-1).sigmoid()
            # bbox_pred = bbox_pred.permute(1, 2, 0).reshape(-1, 8)
            bbox_pred = bbox_pred.permute(1, 2, 0).reshape(-1, 5)
            # bbox_pred = bbox_pred[[1, 0, 13, 12, 17, 16, 5, 4], :, :].permute(1, 2, 0).reshape(-1, 8)
        ...
        mlvl_bboxes = torch.cat(mlvl_bboxes) + 1
        if rescale:
    ...
    def get_targets(self, anchor_list, valid_flag_list, bbox_preds,
        ...
        (all_anchors, all_labels, all_label_weights, all_bbox_targets,
         all_bbox_weights, pos_inds_list, neg_inds_list) = multi_apply(
             ...
             r_gt_bboxes_five_list,
             gt_bboxes_ignore_list,
             gt_labels_list,
             img_metas,
             label_channels=self.cls_out_channels,
             unmap_outputs=True)
        labels_init_list, bbox_targets_list = all_labels, all_bbox_targets
        ...
        return concat_lvl_labels_init, concat_lvl_bbox_targets, concat_lvl_labels, concat_lvl_labels_score, concat_lvl_labels_weight
    ...
    def _get_bbox_target_single(self,
        ...
        if len(gt_bboxes) == 0:
            return flat_anchors, \
                   ...
                   gt_bboxes.new_zeros(flat_anchors.shape[0], 5), \
                   gt_bboxes.new_zeros(flat_anchors.shape[0], 5), \
                   None, None
        ...
        bbox_targets = anchors.new_zeros([num_valid_anchors, 5], dtype=torch.float)
        bbox_weights = anchors.new_zeros([num_valid_anchors, 5], dtype=torch.float)
    ...
    def get_refine_anchors(self,
        ...
        for img_id, img_meta in enumerate(img_metas):
            mlvl_refine_anchors = []
            for i in range(num_levels):
                # bbox_pred = bbox_preds[i][img_id].detach().permute(1, 2, 0).reshape(-1, 8)
                bbox_pred = bbox_preds[i][img_id].detach().permute(1, 2, 0).reshape(-1, 5)
                anchors_perlevel = anchors[img_id][i]
                refine_anchors_perlevel = self.bbox_coder.decode(anchors_perlevel, bbox_pred)
