...
def anchor_target_rotated(anchor_list,
    ...
    (all_labels, all_label_scores, all_label_weights,
    ...
    return (all_labels, all_label_scores, all_label_weights, None, None)
...
def anchor_target_single_obb_obb_rbox(flat_anchors,
    ...
    anchors = flat_anchors[inside_flags, :]
    if len(gt_bboxes) == 0:
        return gt_labels.new_full((flat_anchors.shape[0],), label_channels), \
               anchors.new_zeros((flat_anchors.shape[0],), dtype=torch.float), \
    ...
    labels = anchors.new_full((num_valid_anchors,),
                              ...
                              dtype=torch.long)
    label_weights = anchors.new_zeros(num_valid_anchors, dtype=torch.float)
    label_scores = assign_result.max_overlaps
    pos_inds = sampling_result.pos_inds
    neg_inds = sampling_result.neg_inds
    if len(pos_inds) > 0:
        # pos_bbox_targets = rbox2delta(sampling_result.pos_bboxes,
    ...
    if unmap_outputs:
        num_total_anchors = flat_anchors.size(0)
        labels = unmap(labels, num_total_anchors, inside_flags)
        label_scores = unmap(label_scores, num_total_anchors, inside_flags)
        label_weights = unmap(label_weights, num_total_anchors, inside_flags)
        # bbox_targets = unmap(bbox_targets, num_total_anchors, inside_flags)
        # bbox_weights = unmap(bbox_weights, num_total_anchors, inside_flags)
    ...
    # return (labels, label_weights, bbox_targets, bbox_weights, pos_inds,
    #         neg_inds)
    return (labels, label_scores, label_weights, pos_inds, neg_inds)
