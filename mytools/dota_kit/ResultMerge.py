...
def py_cpu_nms_poly_withconfi(dets, confidence, thresh):
    scores = dets[:, 8] * confidence
    polys = []
    areas = []
    for i in range(len(dets)):
        tm_polygon = polyiou.VectorDouble([dets[i][0], dets[i][1],
    ...
    while order.size > 0:
        ...
        # choose = np.argmax(confidence[order[inds]])
        # keep.append(order[inds[choose]])
        inds = np.where(ovr <= thresh)[0]
        # print('inds: ', inds)
...
if __name__ == '__main__':
    mergebypoly(r'/home/dj/code/RoITransformer/output/rcnn/DOTA16/resnet_v1_101_dotacls16_RoITrans_end2end/test/Task1_results',
                r'/home/dj/code/RoITransformer/output/rcnn/DOTA16/resnet_v1_101_dotacls16_RoITrans_end2end/test/Task1_results_0.1_nms_single')
    # mergebyrec()