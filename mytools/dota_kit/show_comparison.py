...
from mmdet.core import draw_boxes_only, draw_boxes_with_label_and_scores
import cv2
import mmcv
...
def show_the_results(image_id, classname, BB, BBGT, tp, miss, confidence, thresh=0.3):
    img_path = r'/home/nieguangtao/programing/mmdetection/data/DOTA_train/images/{:s}.png'
...
def show_rbox_factor(image_id, BBGT):
    img_path = r'/home/nieguangtao/programing/mmdetection/data/DOTA_train/images/{:s}.png'
    image = img_path.format(image_id)
    factor_score = factor_target(BBGT)
    img_array = cv2.imread(image)
    ...
    labes = np.ones(shape=[len(BBGT), ], dtype=np.float32) * -1
    img_array = draw_boxes_with_label_and_scores(img_array, BBGT, labes, factor_score, method=1, labelname=None)

    show_scale = 500.0 / np.max(img_array.shape)
    img_array = cv2.resize(img_array, (0,0), fx=show_scale, fy=show_scale, interpolation=cv2.INTER_LINEAR)
    # cv2.imshow("show", img_array)
    # cv2.waitKey(0)
    # print("done!")
...
def factor_target(coordinate):
    ...
    for rect in coordinate:
        box = np.int0(rect)
        box = box.reshape([4, 2])
        xymax = np.max(box, axis=0)
        xymin = np.min(box, axis=0)
        rect1 = cv2.minAreaRect(box)
        area_h = xymax-xymin
        area_h = area_h[0] * area_h[1]

        x, y, w, h, theta = rect1[0][0], rect1[0][1], rect1[1][0], rect1[1][1], rect1[2]
        area_r = w * h

        factor.append(area_r/area_h)

    return np.array(factor, dtype=np.float32)

def parse_det(detfile):
    # detfile = detpath.format(filename)
    with open(detfile, 'r') as f:
        lines = f.readlines()
    splitlines = [x.strip().split(' ') for x in lines]
    confidences = np.array([float(x[-2]) for x in splitlines])
    BBs = np.array([[float(z) for z in x[:8]] for x in splitlines])
    results = BBs[confidences >= 0.3]
    return results

def show_all_groundtruth(detpath1, detpath2, annopath, imagesetfile):
    with open(imagesetfile, 'r') as f:
        lines = f.readlines()
    imagenames = [x.strip() for x in lines]
    for imagename in imagenames:
        print(imagename)
        recs = parse_gt(annopath.format(imagename))
        gt_bbox = np.array([x['bbox'] for x in recs])
        gt_img = show_rbox_factor(imagename, gt_bbox)
        # cv2.imshow("gt", gt_img)
        # cv2.waitKey(0)
        det_bbox = parse_det(detpath1.format(imagename))
        det_img = show_rbox_factor(imagename, det_bbox)

        det_bbox = parse_det(detpath2.format(imagename))
        det_img_best = show_rbox_factor(imagename, det_bbox)
        # cv2.imshow("det", det_img)
        # cv2.waitKey(0)
        show = np.hstack((gt_img, det_img))
        show = np.hstack((det_img_best, show))
        cv2.imshow("img", show)
        cv2.waitKey(0)
...
def voc_eval(detpath,
    ...
    choose = ['P1390', 'P1179', 'P1397', 'P2791', 'P1184', 'P0168', 'P1384', 'P2271', 'P1154']
...
if __name__ == '__main__':
    ...
    detpath = os.path.join(r'/home/nieguangtao/programing/new/mmdetection/work_dirs/fcos/Res101_caffe_deformable_train4/det_img_result') + '/{:s}.txt'
    detpath_best = os.path.join(
        r'/home/nieguangtao/programing/new/s2anet/work_dirs/s2anet_r101_fpn_1x_ms_rotate/det_img_result') + '/{:s}.txt'
    annopath = r'/home/nieguangtao/programing/new/mmdetection/data/DOTA_train/labelTxt/{:s}.txt'
    imagesetfile = r'/home/nieguangtao/programing/new/mmdetection/data/DOTA_train/bug.txt'
    # eval_DOTA_Task1_multi_process(detpath, annopath, imagesetfile)
    show_all_groundtruth(detpath, detpath_best, annopath, imagesetfile)
